...
export const Card = ({
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.2 }}
    >
      {image && <img src={image} alt={title} className={`w-full object-cover h-36 ${imageClasses}`} />}
      <div className={`flex items-center justify-between w-full ${padding ? "mt-2" : ""}`}>
        <div className="flex flex-col">
          {title && <h3 className="text-sm md:text-xl font-bold">{title}</h3>}
          {description && <p className="text-xs md:text-base">{description}</p>}
        </div>
        {tag && <Tag label={tag} />}
      </div>
      {footer && <div className="md:mt-4 text-xs flex justify-end w-full">{footer}</div>}
    </motion.div>
  );
};